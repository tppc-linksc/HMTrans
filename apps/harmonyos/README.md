...
当前应用市场公开版本为 `v0.1`。待提交审核的稳定版为 `v0.2.1`（`versionCode=5`）；必须使用包含最终回执、安装级配对身份和双端实时解除配对修复的最新软件包。本目录已完成自有隐私门禁、持久任务、断点恢复、文件夹还原、多设备发送、任务控制和活动传输后台保护。开发分支的 `v0.3.0` 使用 `versionCode=6`，已接入 MatePad 到 Mac 投屏、Mac 主动投屏请求、最多双路独立接收和投屏网络测试并通过 HAP 构建，仍需两台真机完成系统授权、双路采集、后台、方向变化与稳定性验收；它不属于待提交的 v0.2.1 审核包。
...
- 正式签名包和应用市场包仍需持续回归首次启动、后台、锁屏、浮窗/竖屏、深色模式、三设备和超大文件。

## v0.3 HarmonyOS 开发方向

- 用户从已连接 Mac 设备卡片发起投屏，并逐次完成系统屏幕采集授权。
- 设置页可测试 Pad 到 Mac 的实际投屏吞吐、延迟与抖动；测试只发送内存数据。
- 一台 Mac 最多同时接收两台 Pad，双路必须使用不高于 1080P 级 / 30P。
- 使用 HarmonyOS 原生屏幕采集与硬件 H.264 编码，通过独立 HMCS 协议发送到 Mac。
- 支持方向变化、画质策略、会话状态、停止、有限重连和脱敏诊断。
- 投屏不进入文件历史、不生成视频文件，也不改变下载目录授权逻辑。
- 声音、触控、键盘和鼠标反向控制留到后续 `v0.3.x`。

当前设计与实施基线：

- `docs/v0.3-投屏实现说明.md`
- `../../docs/02-v0.3产品与交互方案.md`
- `../../docs/03-v0.3技术方案.md`
- `../../docs/04-v0.3投屏协议.md`
- `../../docs/05-v0.3开发计划.md`
- `../../docs/06-v0.3验收清单.md`
