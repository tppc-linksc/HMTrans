...
- 应用市场首次启动稳定性：等待 AppGallery Connect 托管隐私流程完成后再执行需要用户交互的初始化，避免权限请求无弹窗直接返回。
- 下载目录初始化：HarmonyOS 6.1 / API 23+ 优先按当前系统文件能力直接初始化 `Download/HMTrans`，不把旧式下载目录权限申请作为应用启动和设备发现的硬门槛。
- 服务解耦：UDP 设备发现、TCP 接收服务和下载目录分别维护状态；目录不可用时仍可发现设备，并在界面明确展示“可发送、暂不可接收”及具体原因。
- 权限恢复：支持首次拒绝、设置中重新授权、应用回到前台和版本升级后的重新检查与重试，不要求用户卸载重装。
- UDP 发现可靠性：显式启用广播选项，按当前网卡和子网计算广播地址，记录绑定和发送失败，不静默吞错。
- 应用市场真机验收：必须使用应用市场安装包做干净安装、首次启动、隐私同意、拒绝后恢复和双端发现测试，不能只以 DevEco 调试包通过为准。
- 一对多连接和群组发送。
