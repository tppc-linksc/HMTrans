        ...
        VStack(spacing: 18) {
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 1) {
                    ...
                    Text("v\(Bundle.main.object(forInfoDictionaryKey: "CFBundleShortVersionString") as? String ?? "0.2.1")")
...
private struct MacPortEditorSheet: View {
    ...
    private var portsAreValid: Bool {
        guard let udp = UInt16(udpPort), udp > 0,
              let tcp = UInt16(tcpPort), tcp > 0 else { return false }
        return true
    }
