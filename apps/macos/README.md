...
当前稳定版为 `v0.2.1`。本目录实现了持久任务、暂停/继续/取消、断点恢复、六位码配对、文件夹还原、多设备并发发送、诊断、单实例保护和状态栏快捷入口。开发分支的 `v0.3.0` 已实现投屏接收、H.264 解码、独立观看窗口、画中画和全屏，并通过 macOS 构建与协议测试；真机验收完成前不作为稳定版能力发布。
...
- 当前代码、构建和协议测试已完成，仍需用真实 MatePad 完成首帧、方向切换、长时间投屏和后台场景验收。
