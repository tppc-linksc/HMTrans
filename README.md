...
`v0.1` 已完成首次分发；仓库当前代码为 `v0.2.0` 发布候选实现。v0.2 已落地本地持久化、断点恢复、任务控制、错误诊断、文件夹还原、多设备发送、自有隐私流程和活动传输后台保护。正式发布仍需完成应用市场干净安装、超大文件、锁屏后台和三台真机拓扑压力验收。项目始终只做局域网本地传输，不增加云端和账号系统。
...
- 首次连接必须输入对端 180 秒有效的六位配对码；已配对设备可按设置自动接收或逐文件确认。
...
当前公开分发版本仍是 `v0.1`。仓库中的 `v0.2.0` 已完成双端编译和基础真机互传验证，但在发布清单中的正式渠道干净安装、超大文件、长时间后台/锁屏、三台真机和性能能耗项目完成前，不视为已经发布。
