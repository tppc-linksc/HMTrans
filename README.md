...
当前版本已经能覆盖我大约 95% 的日常使用场景，`v0.1` 已进入发行包真机验证阶段，但未做深度测试和代码深度审核，可能有一些小的使用体验上的问题，也不承诺适合所有网络环境。后续会继续做 `v0.2`，不过短期内有其他事情要忙不会高频更新。欢迎有同样需求的人一起开发、测试和优化。
...
当前代码处于 `v0.1` 发行包验证阶段。Mac 端可生成用于 GitHub Release 分发的 `HMTrans.dmg`，HarmonyOS 端已通过应用市场分发，当前仅支持 Pad 端。
...
`v0.2` 的定位是“多设备、失败恢复与发行版稳定性”：

- 修复 HarmonyOS 应用市场版首次启动时，托管隐私流程与下载目录初始化并发导致权限请求无弹窗返回、接收服务和设备发现无法启动的问题。
- 设备发现与下载目录、TCP 接收服务解耦；即使接收目录暂不可用，也要继续发现设备并明确展示当前能力和失败原因。
- HarmonyOS 6.1 / API 23+ 按当前系统文件能力初始化 `Download/HMTrans`，不再把旧式下载目录权限申请作为发现服务的强制前置条件。
- UDP 广播显式启用广播选项，按当前网卡计算广播地址，并保留可见错误和诊断信息，不再静默吞掉发送失败。
- 一对多连接和群组发送。
...
- HarmonyOS `v0.1` 应用市场版首次启动可能因托管隐私流程与下载目录权限申请时序冲突，导致下载目录未初始化，并进一步阻断 TCP 接收和 UDP 设备发现；该问题已纳入 `v0.2` 统一修复。
